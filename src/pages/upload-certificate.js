...
const UploadCertificate = () => {
    ...
    try {
      if (certificateNumber && selectedFile) {
        ...
        const certificateResponse = await fetch(
          `${apiUrl}/api/verify-certification-id`,
          {
            method: "POST",
            headers: {
      ...
      if (certificateNumber && selectedFile) {
        ...
        if (certificateResponse.ok) {
          ...
          if (fileResponse.ok) {
            ...
            // Check if the certificate number from the file matches the entered certificate number
            if (fileData?.details["Certificate Number"] === certificateNumber) {
              // Both validations passed, update API data
              setApiData({
                Details: certificateData?.details,
                message: certificateData?.message,
              });
            } else {
              // Certificate number and file data mismatch
              router.push("/mismatch-certificate");
              setLoginError(
                "Certificate Number and Certificate PDF doesn't match"
              );
            ...
            // Handle error from file validation
            const errorData = await fileResponse.json();
            if (
              errorData.message === "Certification has revoked" ||
              errorData.message === "Credential has revoked"
            ...
            // General file validation failure
            router.push("/unable-certificate");
            setLoginError(
              errorData.message ||
                "Unable to verify the certification. Please review and try again."
          ...
          // If the first API call fails (invalid certificate number)
          const errorData = await certificateResponse.json();
          if (errorData.message === "Certification has revoked") {
            router.push("/certificate-revoked");
            return;
          ...
          router.push("/invalid-certificate");
          setLoginError("Invalid certificate number. Please try again.");
          setShow(true);
        }
      } else {
        // Handle error as needed
  ...
  const certificateUrl = `https://testverify.certs365.io/certificate/${
    apiData?.Details["Certificate Number"]
  }?certificatenumber=${apiData?.Details["Certificate Number"]}&coursename=${
    apiData?.Details["Course Name"]
  }&grantdate=${
    apiData?.Details["Grant Date"] || apiData?.Details["grantDate"]
  }&expirationdate=${
    apiData?.Details["Expiration Date"] || apiData?.Details["expirationDate"]
  }&name=${apiData?.Details["Name"]}`;
  const imageUrl = `https://testverify.certs365.io/api/og?certificatenumber=${
    apiData?.Details["Certificate Number"]
  }&coursename=${apiData?.Details["Course Name"]}&grantdate=${
    apiData?.Details["Grant Date"] || apiData?.Details["grantDate"]
  }&expirationdate=${
    apiData?.Details["Expiration Date"] || apiData?.Details["expirationDate"]
  }&name=${apiData?.Details["Name"]}`;
            ...
            <div className="position-relative h-100">
              <div className="vertical-center verify-cert">
                <div className="container-fluid">
                  ...
                  <Row className="justify-content-center mt-3 verify-documents">
                    ...
                    <Col md={{ span: 10 }}>
                      <Card className="p-4">
                        <Row className="card-certificate ">
                          <Form
                            className="form-certificate-number card-certificate "
                            style={{
                              display: "flex",
                        ...
                        <div className="divider position-relative text-center">
                          and
                        </div>
